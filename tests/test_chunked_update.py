...
from _multilang import count_graphemes, normalize, ensure_rtl_integrity
...
class TestMultilanguageIntegration:
    """Multi-language chunking: Arabic RTL, Japanese CJK, Korean Hangul."""

    def test_chinese_chunking_preserves_content(self):
        """Chinese text chunked by grapheme count."""
        content = "# 视频脚本\n\n" + "中文字符" * 200 + "\n\n---\n\n## 结尾\n\n" + "更多内容" * 200
        chunks = split_by_sections(content, 500)
        assert len(chunks) >= 2
        full = "\n\n---\n\n".join(chunks)
        assert "# 视频脚本" in full

    def test_japanese_dakuten_in_chunks(self):
        """Japanese with dakuten chars split cleanly by grapheme."""
        # が = 1 grapheme, so 150 graphemes of が should fit in chunk_size=200
        content = "# 日本語\n\n" + "が" * 150 + "\n\n---\n\n## 終わり\n\n" + "な" * 150
        chunks = split_by_sections(content, 200)
        assert len(chunks) >= 2
        # Each chunk should be within grapheme limit
        for c in chunks:
            assert count_graphemes(c) <= 200

    def test_korean_hangul_chunking(self):
        """Korean Hangul syllables count as 1 grapheme each."""
        content = "# 한국어\n\n" + "안녕하세요" * 80 + "\n\n---\n\n## 마무리\n\n" + "감사합니다" * 80
        chunks = split_by_sections(content, 400)
        assert len(chunks) >= 2
        for c in chunks:
            assert count_graphemes(c) <= 400

    def test_arabic_rtl_chunking(self):
        """Arabic RTL text chunks with RTL integrity markers."""
        content = "# عربي\n\n" + "مرحبا" * 100 + "\n\n---\n\n## نهاية\n\n" + "شكرا" * 100
        chunks = split_by_sections(content, 400)
        assert len(chunks) >= 2
        for c in chunks:
    ...
    def test_normalize_applied(self):
        ...
        # Decomposed Japanese: か + dakuten -> が
        decomposed = normalize("が")
        assert decomposed == "が"

    def test_rtl_integrity_on_chunk_boundary(self):
        """Chunks ending with unclosed RTL markers get closers appended."""
        unclosed = "Hello ‪مرحبا"  # RLE + Arabic, no PDF
        result = ensure_rtl_integrity(unclosed)
        assert result.endswith("‬")  # PDF appended
