...
from _multilang import count_graphemes, ensure_rtl_integrity, normalize
from _multilang import _get_graphemes as _get_graphemes
...
示例:
    ...
    parser.add_argument(
        "--encoding",
        default="utf-8",
        help="文件编码（默认 utf-8；支持 utf-8-sig、gbk、cp949 等）",
    )
...
def split_by_sections(content: str, chunk_size: int = 4000) -> List[str]:
    ...
    for section in sections:
        ...
        if count_graphemes(current_chunk) + count_graphemes(section) + 4 > chunk_size:
            if current_chunk:
                chunks.append(ensure_rtl_integrity(current_chunk))

            if count_graphemes(section) > chunk_size:
...
def split_large_section(section: str, chunk_size: int) -> List[str]:
    ...
    for para in paragraphs:
        if count_graphemes(para) > chunk_size:
            if current_chunk:
                sub_chunks.append(ensure_rtl_integrity(current_chunk))
                current_chunk = ""

            lines = para.split("\n")
            temp_chunk = ""
            for line in lines:
                # Hard-cut: single line exceeds chunk_size → split by grapheme
                if count_graphemes(line) > chunk_size:
                    if temp_chunk:
                        sub_chunks.append(ensure_rtl_integrity(temp_chunk))
                        temp_chunk = ""
                    graphemes = _get_graphemes(line)
                    line_buf = ""
                    for g in graphemes:
                        if count_graphemes(line_buf) + 1 > chunk_size:
                            sub_chunks.append(ensure_rtl_integrity(line_buf))
                            line_buf = g
                        else:
                            line_buf += g
                    if line_buf:
                        sub_chunks.append(ensure_rtl_integrity(line_buf))
                elif count_graphemes(temp_chunk) + count_graphemes(line) + 1 > chunk_size:
                    if temp_chunk:
                        sub_chunks.append(ensure_rtl_integrity(temp_chunk))
                    temp_chunk = line
                else:
            ...
            if temp_chunk:
                sub_chunks.append(ensure_rtl_integrity(temp_chunk))
        elif count_graphemes(current_chunk) + count_graphemes(para) + 2 > chunk_size:
            sub_chunks.append(ensure_rtl_integrity(current_chunk))
            current_chunk = para
        else:
            if current_chunk:
...
def main() -> int:
    ...
    try:
        raw_content = file_path.read_text(encoding=args.encoding)
    ...
    logger.info("编码: %s", args.encoding)
    logger.info("总字符数: %d (可视化单元: %d)", len(content), count_graphemes(content))
